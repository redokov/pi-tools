...
index.ts ──── оркестратор: хуки (session_start, model_select,
  ...
  │                  repeat-режим (перевзвод до N срабатываний).
  │
...
| `/cont-after-reset [N\|off]` | Автопродолжение после сброса; `N` (1..99) — на N срабатываний, `off` — снять. См. раздел 8a. |
...
- `arms.ts` — взвод/снятие/идемпотентность, TTL и prune, repeat-режим (перевзвод `repeat - 1` на `confirmSuccess`), фазы `armed`/`pending` и ретрай при 429, перенос флага при `/new` (`carryArmTo`), переключение ключа без переноса (`switchKey`), `resetReadyToFire` (граница grace, срабатывание только после взвода), атомарность записи
...
- `/cont-after-reset N` — взвести флаг на N срабатываний (N = 1..99).
  При N > 1 после подтверждённого срабатывания флаг не снимается, а
  перевзводится: `repeat` уменьшается на 1, `lastResetAtAtArm` становится
  последним сбросом, TTL отсчитывается заново (8 ч). Так одна взведённая
  ночная сессия переживает несколько 2-часовых окон подряд без ручного
  перевзвода.
...
- затем, если агент не стримит, в чат отправляется одно слово `продолжи`
  ...
  пробуем снова;
- после отправки флаг переходит в фазу `pending`: при 429 от провайдера
  `продолжи` повторяется не чаще раза в 10 мин, а первый успешный ответ
  подтверждает флаг — одноразовый снимается, при `repeat > 1` перевзводится
  на следующий сброс;
- продолжается только взведённое окно. Остальные окна сброс просто отражают
  в счётчике.

Ограничения:
- срок годности флага — 8 часов от взвода (если сброса за это время не
  было, флаг сгорает); в repeat-режиме TTL отсчитывается заново от каждого
  перевзвода;
...
Заметка (мотивация repeat-режима): одноразовый флаг уже приводил к ночному
простою. В ночь 18–19.09 флаг сработал однократно, после первого `продолжи`
сгорел, агент упёрся в 429, и последующие сбросы окна (02:15 / 04:15 / 06:15)
не стреляли — больше 6 часов простоя до ручного вмешательства. Repeat-режим
(`/cont-after-reset N`) закрывает именно этот сценарий.

### Самовзведение флага агентом (ночная работа)

При обычном взводе флаг одноразовый — после срабатывания и подтверждения он
сгорает. Для автономной ночной работы (агент без человека), когда одного
сброса окна мало, используется repeat-режим: флаг взводится с числом
продолжений и после каждого подтверждённого срабатывания перевзводится, пока
счётчик не исчерпается. Взвести такой флаг можно командой
`/cont-after-reset N` либо прямой записью в
`~/.pi/agent/pi-billing-window-arms.json` под ключом сессии — так делает
helper-скрипт проекта (`scripts/arm_cont_after_reset.py` в TabDocLoad,
поддерживает `--repeat N`; скрипт пишет поле `repeat` в arms.json, а
расширение подхватывает его живым poller'ом). Для этого расширение:

- запускает armed-poller на каждый `session_start` (не только после
  `/cont-after-reset`);
- не останавливает poller, когда флага нет — «пустые» тики (10 c, чтение
  двух JSON) стоят копейки, зато внешняя запись в arms-файл подхватывается
  немедленно.
