...
Флаг одноразовый — после срабатывания и подтверждения он сгорает. Для
автономной ночной работы (агент без человека) флаг можно взводить
прямой записью в `~/.pi/agent/pi-billing-window-arms.json` под ключом
сессии — так делает helper-скрипт проекта (например,
`scripts/arm_cont_after_reset.py` в TabDocLoad). Для этого расширение:
