...
type Arm = {
  ...
  expiresAt: number;       // ms epoch = armedAt + ARMS_TTL_MS (8 часов)
  phase?: "armed" | "pending"; // "armed" — ждём сброс; "pending" — "продолжи" отправлен, ждём первый успешный ответ
  lastFireAt?: number;     // ms epoch — когда отправлен последний "продолжи" (только в фазе "pending")
  repeat?: number;         // сколько всего срабатываний допускает флаг (отсутствует/1 = одноразовый);
                           // при repeat > 1 подтверждённый успех перевзводит запись с repeat - 1
};
...
В repeat-режиме (`repeat > 1`) в записи дополнительно присутствуют поля `phase`, `lastFireAt` и `repeat`; после подтверждённого срабатывания запись не удаляется, а перевзводится (см. §8.2).

### 8.2. Жизненный цикл записи

| Событие | Что происходит с записью |
|---|---|
| `/cont-after-reset` | Создаётся под ключом текущей сессии (идемпотентно: активную запись не перезаписывает). С числовым аргументом `N` — перевзводится с `repeat = N`. |
| Сброс окна + 60 с grace, агент idle | Отправлено `продолжи` → запись переходит в фазу `pending` (`phase`, `lastFireAt`); TTL при этом не сокращается. |
| Первый успешный ответ провайдера (фаза `pending`) | Одноразовый флаг (`repeat` отсутствует или 1) удаляется; при `repeat > 1` запись перевзводится: `repeat - 1`, `phase = "armed"`, `lastResetAtAtArm` = текущий `state.lastResetAt`, `expiresAt = now + 8 ч`, `lastFireAt` снят. |
