...
## 11. Ручная верификация cont-after-repeat

Проверка repeat-режима `cont-after-reset` требует живой сессии pi, куда безопасно отправить `продолжи`. Порядок:

1. В живой сессии pi взвести флаг с повтором — `/cont-after-reset 2` (либо из проекта-скрипта: `python scripts/arm_cont_after_reset.py --repeat 2`).
2. `/settimer 0` — мгновенный сброс окна (полный reset с продвижением `lastResetAt`).
3. Через ~60 с (`RESET_GRACE_MS`) в чат прилетает `продолжи` — расширение видит сброс и, если агент idle, отправляет сообщение.
4. После первого успешного ответа провайдера открыть `~/.pi/agent/pi-billing-window-arms.json` и проверить запись своей сессии: запись жива, `repeat` уменьшился на 1, `phase` снова `"armed"`, `lastResetAtAtArm` равен последнему сбросу.
5. Повторить `/settimer 0` — убедиться, что пришло второе `продолжи`, а при исчерпании счётчика (`repeat` дошёл до 1) запись после подтверждения удалена.

Важно: шаги 3 и 5 отправляют в чат реальное `продолжи` — тестировать только в сессии, где это безопасно (не на боевой задаче).
