...
- **arms.ts** — per-разговор флаги `/cont-after-reset` в отдельном файле (см. §5 и [`STATE.md`](./STATE.md) §8). Тот же паттерн, что и state.ts: атомарная запись + `proper-lockfile`. От state.ts не зависит — только index.ts и ui.ts его используют.
