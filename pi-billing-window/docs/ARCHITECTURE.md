...
- **state.ts** — основной модуль I/O для общего состояния. Все остальные читают/пишут его только через `readStateSync()` и `mutateState()`. Это позволяет держать инвариант «state всегда либо валидный, либо `null`» в одном месте.
- **arms.ts** — per-разговор флаги `/cont-after-reset` в отдельном файле (см. §5 и [`STATE.md`](./STATE.md) §8). Хранит фазу (`armed`/`pending`), TTL и необязательный `repeat`; при `repeat > 1` `confirmSuccess` перевзводит запись (`repeat - 1`, `armed`, TTL 8 ч заново) — флаг переживает несколько сбросов. Тот же паттерн, что и state.ts: атомарная запись + `proper-lockfile`. От state.ts не зависит — только index.ts и ui.ts его используют.
- **history.ts** — append-only CSV-история вызовов/сбросов (см. §6 и [`STATE.md`](./STATE.md) §9): строки только дописываются, правок нет (кроме ретеншн-трима раз в сессию). Никогда не бросает — сбой истории не влияет на state/тикер/уведомления. Отдельный лок (не общий со state): критическая секция — одна строка.
...
/cont-after-reset
   ...
   │                    armsMarkFired(): флаг → "pending"
   ├── фаза "pending" → ждём первый успешный ответ провайдера
   │      (429 → повтор отправки каждые RETRY_AFTER_FIRE_MS = 10 мин)
   │      └── confirmSuccess(): repeat>1 → перевзвод (repeat-1, "armed",
   │          TTL 8 ч заново, lastResetAtAtArm = последний сброс);
   │          иначе → запись удалена
...
|---|---|
| `/cont-after-reset` / `/cont-after-reset N` | Взвод: без аргумента — идемпотентно (активный флаг не перезаписывается); явный `N` (1..99) перевзводит с `repeat = N`. |
| `/cont-after-reset off` | Снятие + остановка poller'а. |
| Сброс окна + 60 с | Отправка `продолжи`, флаг → `pending` (ждём первый успешный ответ). |
| Первый успешный ответ (фаза `pending`) | `confirmSuccess`: одноразовый флаг удаляется; при `repeat > 1` — перевзвод (`repeat - 1`, `armed`, TTL 8 ч заново, `lastResetAtAtArm` = последний сброс). |
| TTL 8 часов | Флаг истекает (`expiresAt`), игнорируется и вычищается при следующей записи в файл. |
...
## 6. История вызовов (history.csv)

Passive-наблюдатель сбоку: ничего не меняет в поведении расширения, только дописывает строки. Файл **один на все pi-процессы** — лимит wormsoft аккаунтный, поэтому аналитика нужна и по проектам, и суммарная. Проект — колонка `project` (полный cwd сессии), а не отдельный файл: избегаем коллизий имён, кириллицы в именах файлов и склейки при суммарном анализе.
...
## 8. Зависимости и порядок инициализации
...
4. На `session_start` (порядок продуман: таймеры запускаются **до** state-I/O, чтобы сбой бутстрапа не оставлял сессию без отсчёта):
   ...
   5. трим history-файла (строки старше 30 дней) — fire-and-forget.
...
| history-файл недоступен (диск, права, антивирус) | `appendHistory`/`trimHistory` возвращают false/0 и пишут warn; calls/reset/уведомления работают как обычно. |
...
## 10. Что можно вынести за пределы расширения
